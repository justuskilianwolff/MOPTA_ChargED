...
cwd = os.getcwd()
file = "small_car_locations.csv"  # full_car_locations.csv / small_car_location.csv
if "car_locations" not in st.session_state:
    st.session_state["car_locations"] = pd.read_csv(
        os.path.join(cwd, "locations", file)
    )
...
if "optimise_status" not in st.session_state:
    st.session_state["optimise_status"] = (
        0  # 0: not optimised, 1: in progress, 2: finished
    )
...
# Own Data Selection
own_data = st.sidebar.checkbox(
    "Import Vehicle Locations?"
)  # Tick if want to upload data
if own_data:
    uploaded_file = st.sidebar.file_uploader(
        'Upload your .csv file here (same format as default file, with "x" and "y" columns only, 1 row per vehicle)',
    ...
    if uploaded_file is not None:
        ...
        with map_cont.container():
            vehicle_locations = go.Scatter(
                x=car_locations.iloc[:, 0],
                y=car_locations.iloc[:, 1],
                mode="markers",
                name="Vehicle Locations",
                marker=dict(size=2, color="blue"),
            )
            x_axis = go.XAxis(
                title="x coordinate / Miles",
                showgrid=False,
                mirror=True,
                ticks="outside",
                showline=True,
            ...
            layout = go.Layout(
                title_text="Vehicle Locations", xaxis=x_axis, yaxis=y_axis, height=500
            )  # title_x=0.5,
...
fixed_chargers = st.sidebar.checkbox(
    "Specify Total Stations Built?",
    help="Check this box if you want to specify the number of stations built",
)
if fixed_chargers:
    fixed_station_number = st.sidebar.slider(
        "How many stations should be built?",
        min_value=1,
    ...
    st.session_state["fixed_station_number"] = (
        fixed_station_number  ## Add to session state
    )

# Vary upper bound of number of stations at each location
station_ub = st.sidebar.slider(
    "Maximum Stations at any given Location:",
    min_value=1,
    max_value=20,
    step=1,
    value=8,
...
c_b = st.sidebar.number_input(
    "Annualised Construction Cost (Per Location)", min_value=0, value=5000
)
c_m = st.sidebar.number_input(
    "Annualised Maintenence Cost (Per Station)", min_value=0, value=500
)
c_d = st.sidebar.number_input(
    "Drive cost per mile", min_value=0.0, value=0.041, format="%0.5f"
)
c_c = st.sidebar.number_input(
    "Charge cost per mile", min_value=0.0, value=0.0388, format="%0.5f"
)
...
st.session_state["number_of_samples"] = st.sidebar.number_input(
    "Number of Samples Optimised Over",
    min_value=1,
    value=3,
...
drive_charge_cost_metric = col2.metric(
    "Drive & Charge Cost", st.session_state["drive_charge_cost"]
)
built_locs_metric = col3.metric("Locations Built", st.session_state["locations_built"])
built_chargers_metric = col4.metric(
    "Chargers Built", st.session_state["chargers_built"]
)
...
total_cost_iter = []
drive_charge_cost_iter = []
chargers_built_iter = []
locations_built_iter = []
...
def streamlit_update(solver):
    ...
    L_new = solver.added_locations[
        -1
    ]  # [-1] for locations added in most recent iteration
    b, n, u = solver.solutions[
        -1
    ]  # b = build binary vars, n = number of chargers built, u = allocation matrix
    ...
    with map_cont.container():
        ...
        built_locations = go.Scatter(
            x=results_df[results_df["b"] == 1].loc_x,
            y=results_df[results_df["b"] == 1].loc_y,
            mode="markers",
            name="Built Locations",
            marker=dict(size=results_df[results_df["b"] == 1].n, color="red"),
        )
        considered_locations = go.Scatter(
            x=results_df[results_df["b"] == 0].loc_x,
            y=results_df[results_df["b"] == 0].loc_y,
            mode="markers",
            name="Considered Locations (Not Built)",
        ...
        layout = go.Layout(
            title_text="Vehicle & Charger Locations (Built and Considered)",
            xaxis=x_axis,
            yaxis=y_axis,
            height=500,
        )  # title_x=0.5
        map_fig = go.Figure(
            data=[vehicle_locations, built_locations, considered_locations],
            layout=layout,
        )
    ...
    #################################
    ## METRIC PANEL UPDATE
    locations_built = results_df["b"].sum()
    locations_built_iter.append(locations_built)
    chargers_built = results_df["n"].sum()
    chargers_built_iter.append(chargers_built)
    st.session_state["locations_built"] = locations_built
    st.session_state["chargers_built"] = chargers_built

    # Total Cost KPI
    total_cost = solver.m.kpi_value_by_name(
        name="total_cost"
    )  ## 'build_cost', 'maintenance_cost', 'drive_charge_cost'
    ...
    total_cost_metric.metric(
        ...
        "$" + str(np.round(total_cost_improvement, 0)),
    )  # delta_color="inverse"
    drive_charge_cost_metric.metric(
        "Drive Charge Cost", "$" + str(int(st.session_state["drive_charge_cost"]))
    )
    ...
    with total_cost_cont.container():
        ...
        total_cost_fig.update_layout(
            title_text="Total Cost / $", title_x=0.5, showlegend=False
        )
    ...
    with other_cost_cont.container():
        other_cost_fig = px.line(
            drive_charge_cost_iter, template="simple_white", markers="x"
        )
        other_cost_fig.update_xaxes(
            title="Iteration",
            showgrid=False,
            mirror=True,
            ticks="outside",
        ...
        other_cost_fig.update_yaxes(
            title="Drive-Charge Cost /$",
            showgrid=False,
            mirror=True,
            ticks="outside",
            showline=True,
        )
        other_cost_fig.update_layout(
            title_text="Drive-Charge Cost / $", title_x=0.5, showlegend=False
        )
    ...
    with charger_count_cont.container():
        ...
        charger_count_fig.update_xaxes(
            title="Chargers at Location",
            showgrid=False,
            mirror=True,
            tickvals=[1, 2, 3, 4, 5, 6, 7, 8],
        )
        charger_count_fig.update_yaxes(
            title="Number of Locations", showgrid=False, mirror=True, showline=True
        )
        charger_count_fig.update_layout(
            title_text="Number of Chargers by Location", title_x=0.5, showlegend=False
        )
...
mopta_solver = MOPTASolver(
    car_locations=car_locations.to_numpy(),
    ...
    streamlit_callback=streamlit_update,
)  # We set streamlit_update as the callback function
...
if st.button(
    "Optimise...",
    help="Start the optimiser with the parameters selected in the sidebar",
):
    ...
    if st.session_state["fixed_station_number"] is None:
        ...
        number_of_initial_locations = int(
            max(
                np.ceil(len(car_locations) / 10),
                st.session_state["fixed_station_number"],
            )
        )  # 400
    mopta_solver.add_initial_locations(
        number_of_initial_locations, mode="k-means", seed=0
    )  # mode='random'
    # Add User-Specified number of Samples
    mopta_solver.add_samples(num=st.session_state["number_of_samples"])
    # Call the solve function
    n, L = mopta_solver.solve(
        verbose=False, timelimit=time_limit, epsilon_stable=epsilon_stable
    )
    # Update Session State with n and L
    st.session_state["n_sol"] = n
    st.session_state["L_sol"] = L
    ...
    for i in range(2, iterations + 1):
        ...
        )  # start with the initial locations
        for j in range(1, i):
            location_data = pd.concat(
                [location_data, pd.DataFrame(location_iterations[j])], axis=0
            )
    ...
    st.session_state["solution"] = sol_df
    st.write(
        f'Finished Solving after {iterations} Iterations.Solution exported as "sol_df.csv" file.'
    )
...
if st.session_state["optimise_status"] == 2:
    total_cost_metric.metric(
        "Total Cost", "$" + str(int(st.session_state["total_cost"]))
    )
    drive_charge_cost_metric.metric(
        "Drive Charge Cost", "$" + str(int(st.session_state["drive_charge_cost"]))
    )
    ...
    with map_cont.container():
        ...
        try:
            final_fig.layout.updatemenus[0].buttons[0].args[1]["frame"]["duration"] = (
                3000  # Change animation speed
            )
        except:
            pass
...
validate_iterations = st.number_input(
    "Once Optimised, test solution against a number of unseen samples, specified below",
    min_value=1,
    max_value=None,
    value=100,
)
...
if st.button(
    "Validate Solution",
    help="Click to test the solution with unseen samples to check robustness",
):
    if st.session_state["optimise_status"] != 2:
        st.write(
            'No solution yet generated. Please click the "Optimise" button to solve for your selected parameters'
        )
    else:
        st.write(
            f"Testing Found Solution on {validate_iterations} Unseen Vehicle Samples."
        )
        objective_values, service_levels, mip_gaps = mopta_solver.allocation_problem(
        ...
        feasible = st.session_state["validate_df"][
            st.session_state["validate_df"]["service_level"]
            == st.session_state["service_level"]
        ].reset_index(drop=True)
        infeasible = st.session_state["validate_df"][
            st.session_state["validate_df"]["service_level"]
            < st.session_state["service_level"]
        ].reset_index(drop=True)
        ...
        st.write(
            f"Of the {validate_iterations} samples tested, {feasible_count} were 'feasible', that is an allocation was found sending 95% of vehicles which needed to charge to a charge station. "
        )
        ...
        with validate_col1:
            ...
            feasible_fig.update_yaxes(
                title="Frequency Density", showgrid=False, mirror=True, showline=True
            )
            feasible_fig.update_layout(
                title_text=f"Histogram of Total Cost for {feasible_count} Feasible Solutions",
                title_x=0,
                showlegend=False,
            )
            st.plotly_chart(feasible_fig, use_container_width=True)
        with validate_col2:
            infeasible_fig = px.scatter(
                infeasible, x="service_level", y="objective", template="simple_white"
            )
            infeasible_fig.update_xaxes(
                title="Service Level", showgrid=False, mirror=True
            )
            infeasible_fig.update_yaxes(title="Total Cost", showgrid=False, mirror=True)
            infeasible_fig.update_layout(
                title_text=f"Service Level vs. Total Cost for {validate_iterations - feasible_count} Infeasible Solutions",
                title_x=0,
                showlegend=False,
            )
